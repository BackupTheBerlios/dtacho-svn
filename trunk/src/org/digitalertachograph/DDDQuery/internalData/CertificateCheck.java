...
public class CertificateCheck {
	...
	private byte[] cipheredCertificate;
	private byte[] decipheredSignature;

	private byte[] c_r = new byte[ 106 ];
	private byte[] c_n = new byte[ 58 ];
	private byte[] c_0 = new byte[ 106 + 58 ];
	private byte[] car = new byte[ 8 ];
	private byte[] h_0 = new byte[ 20 ];
	...
	public void setCipheredCertificate( byte[] cipheredCertificate ) {
		this.cipheredCertificate = cipheredCertificate;
	}
	...
	public void decipherCertificateSignature() {
		try {
			...
			decipheredSignature = mycipher.doFinal( cipheredCertificate, 0, 128 );

			// certificate content, signature part
			// c_r
			System.arraycopy( decipheredSignature, 1, c_r, 0, 106 );

			// certificate content, certificate part
			// c_n
			System.arraycopy( cipheredCertificate, 128, c_n, 0, 58 );
			...
			// car
			System.arraycopy( cipheredCertificate, 186, car, 0, 8 );

			// sha1 hash
			// h_0
			System.arraycopy( decipheredSignature, 107, h_0, 0, 20 );
		}
	...
	public boolean isValidSignature() {
		if ( ( decipheredSignature[ 0 ] == (byte)0x6a ) && ( decipheredSignature[ 127 ] == (byte)0xbc ) ) {
			certificateContent = new CertificateContent( c_0 );
			return true;
		}
		else
			return false;
	...
	public String getC_0SHAHashHexString() {
		try {
			MessageDigest md = MessageDigest.getInstance( "SHA-1" );
			md.update( c_0 );
			return new ByteArrayTools().ByteArrayToHexString( md.digest() );
		}
		catch ( NoSuchAlgorithmException nsae ) {
			return null;
		}
	...
	public String getC_RHexString() {
		return new ByteArrayTools().ByteArrayToHexString( c_r );
	}
	...
	public String getC_NHexString() {
		return new ByteArrayTools().ByteArrayToHexString( c_n );
	}
	...
	public String getC_0HexString() {
		return new ByteArrayTools().ByteArrayToHexString( c_0 );
	}
	...
	public String getH_0HexString() {
		return new ByteArrayTools().ByteArrayToHexString( h_0 );
	}
}
